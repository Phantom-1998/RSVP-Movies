...
select table_name as 'Table Name', table_rows as 'No of Rows'
from information_schema.tables
where TABLE_SCHEMA = 'imdb_movies'
;
...
-- There are four columns in which have null values in movie table.
...
rank() over(order by count(genre) desc) AS 
genre_rank
FROM genre
group by genre
...
SELECT COUNT(movie_id) AS single_genre_movie
FROM single_genre;
